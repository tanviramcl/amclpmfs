...
public partial class UI_ReportViewer_StockDeclarationBeforePostedReportViewer : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (pfolioAsOnDate != "" && Fromdate == "" && Todate == "" && pfolioPreviousMonthDate == "")
        {
            ...
            sbMst.Append(" WHERE      (PFOLIO_BK.SECT_MAJ_CD <> 89) AND (PFOLIO_BK.BAL_DT_CTRL = '"+pfolioAsOnDate+"') AND (FUND.F_CD NOT IN (1, 3, 5, 16, 18))  GROUP BY FUND.F_NAME, FUND.F_CD) EQUITY LEFT OUTER JOIN  (SELECT     FUND_1.F_CD, FUND_1.F_NAME, SUM(PFOLIO_BK_1.TCST_AFT_COM) AS INVEST_DEBT, ");
        ...
        else if (Fromdate != "" && Todate != "" && pfolioAsOnDate != "" && pfolioPreviousMonthDate != "")
        {
            ...
            sbMst.Append("   FROM          FUND_TRANS_HB  WHERE      (VCH_DT BETWEEN '"+Fromdate+"' AND '"+Todate+"') AND (TRAN_TP = 'S')  ");
            ...
            sbMst.Append("   FROM          PFOLIO_BK PFOLIO_BK_1  WHERE      (BAL_DT_CTRL = '"+pfolioPreviousMonthDate+"') AND (F_CD NOT IN (1, 3, 5, 16, 18))  ");
            ...
            sbMst.Append("   FROM          PFOLIO_BK WHERE      (BAL_DT_CTRL = '"+pfolioAsOnDate+"') AND (F_CD NOT IN (1, 3, 5, 16, 18)) GROUP BY F_CD) CLOSING_BALANCE INNER JOIN ");
            sbMst.Append("   (SELECT     F_CD, F_NAME  FROM          FUND FUND_1  WHERE      (F_CD NOT IN (1, 3, 5, 16, 18))) FUND ON CLOSING_BALANCE.F_CD = FUND.F_CD ON OPENING_BALANCE.F_CD = FUND.F_CD ON  ");
            sbMst.Append("   WHERE      (VCH_DT BETWEEN '"+Fromdate+"' AND '"+Todate+"') AND (TRAN_TP = 'C')    GROUP BY F_CD) PURCHASE ON CLOSING_BALANCE.F_CD = PURCHASE.F_CD ORDER BY FUND.F_CD  ");
        ...
        else if (pfolioAsOnDate == "" && Fromdate != "" && Todate != "" && pfolioPreviousMonthDate == "")
        {
            ...
            sbMst.Append("  WHERE     (FUND.F_CD NOT IN (1, 3, 5, 16, 18)) AND (FUND_TRANS_HB.VCH_DT BETWEEN '"+Fromdate+"' AND '"+Todate+"') AND (COMP.SECT_MAJ_CD = 12) AND ");
